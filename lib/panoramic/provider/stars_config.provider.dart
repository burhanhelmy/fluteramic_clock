import 'dart:math';

import 'package:flutter/material.dart';

enum ConfigState { NULL, GENERATED }

class StarsConfigProvider {
  static final StarsConfigProvider _singleton = StarsConfigProvider._internal();

  factory StarsConfigProvider() {
    return _singleton;
  }

  int starsCount = 0;
  ConfigState state = ConfigState.NULL;
  List<StarInfo> starsPositions = [];
  Random rnd = Random();

  StarsConfigProvider._internal();

  generateStarSettings(Size size) {
    starsCount = (50 + rnd.nextInt(100));
    for (var i = 0; i < starsCount; i++) {
      var xpos = (0 + rnd.nextInt(size.width.toInt())).toDouble();
      var ypos = (0 + rnd.nextInt(size.height.toInt() ~/ 2)).toDouble();
      starsPositions.add(StarInfo([xpos, ypos], 0.1));
    }
    print("generated");
    state = ConfigState.GENERATED;
  }

  resetSettings() {
    starsCount = 0;
    starsPositions = [];
    state = ConfigState.NULL;
  }
}

class StarInfo {
  StarInfo(this.coordinate, this.opacity);
  List<double> coordinate = [];
  double opacity = 0;
}
